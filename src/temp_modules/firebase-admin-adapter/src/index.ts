...
// TODO: Fuck it. No emulator support. Soz. 

export function FirestoreAdminAdapter(FirebaseApp: App): Adapter {
    // Assume that the app is already initialized. Absolute stupidity otherwise. 

    const Database = getFirestore(FirebaseApp)
    const Users = Database.collection("users").withConverter(getConverter<AdapterUser>())
    const Sessions = Database.collection("sessions").withConverter(getConverter<AdapterSession & IndexableObject>())
    const Accounts = Database.collection("accounts").withConverter(getConverter<Account>())
    const VerificationToken = Database.collection("verificationTokens").withConverter(getConverter<VerificationToken & IndexableObject>({ excludeId: true }))
    ...
    return {
        async createUser(newUser) {
            ...
            const userRef = await Users.doc().set(newUser)
        },
        async getUser(id) {
